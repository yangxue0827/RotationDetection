...
class DetectionNetworkRetinaNet(DetectionNetworkBase):
    ...
    def rpn_reg_net(self, inputs, scope_list, reuse_flag, level):
        ...
        rpn_angle_sin_ = rpn_angle_sin / tf.sqrt(tf.pow(rpn_angle_sin, 2) + tf.pow(rpn_angle_cos, 2))
        rpn_angle_cos_ = rpn_angle_cos / tf.sqrt(tf.pow(rpn_angle_sin, 2) + tf.pow(rpn_angle_cos, 2))

        rpn_delta_boxes = tf.reshape(rpn_delta_boxes, [-1, 4], name='rpn_{}_regression_reshape'.format(level))
        rpn_angle_sin_ = tf.reshape(rpn_angle_sin_, [-1, 1], name='rpn_{}_sin_reshape'.format(level))
        rpn_angle_cos_ = tf.reshape(rpn_angle_cos_, [-1, 1], name='rpn_{}_cos_reshape'.format(level))

        rpn_delta_boxes = tf.concat([rpn_delta_boxes, rpn_angle_sin_, rpn_angle_cos_], axis=-1)
