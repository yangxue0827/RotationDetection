...
class AnchorSamplerGWD(Sampler):
    ...
    def anchor_target_layer(self, gt_boxes_h, gt_boxes_r, anchors, gpu_id=0):
        ...
        if self.cfgs.ANGLE_RANGE == 180:
            anchors = coordinate_present_convert(anchors, mode=-1, shift=False)
            target_boxes = coordinate_present_convert(target_boxes, mode=-1, shift=False)
