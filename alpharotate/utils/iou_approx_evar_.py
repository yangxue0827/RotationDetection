...
def kf(boxes1, boxes2):
    ...
    mu1 = tf.reshape(tf.concat([x1, y1], axis=-1), [-1, 2, 1])
    mu2 = tf.reshape(tf.concat([x2, y2], axis=-1), [-1, 2, 1])

    mu, sigma= kalman_filter(mu1, mu2, sigma1, sigma2)
    # mu, sigma= kalman_filter(mu2, mu1, sigma2, sigma1)
    ...
    eig = tf.reshape(tf.linalg.eigvalsh(sigma), [-1, 2])
    overlap = tf.sqrt(eig[:, 0] * eig[:, 1]) * 4
    aera1 = boxes1[:, 2] * boxes1[:, 3]
    aera2 = boxes2[:, 2] * boxes2[:, 3]
    ...
    return sigma1, sigma2, iou


def kf2(boxes1, boxes2):
    x1, y1, w1, h1, theta1 = tf.unstack(boxes1, axis=1)
    x2, y2, w2, h2, theta2 = tf.unstack(boxes2, axis=1)
    x1 = tf.reshape(x1, [-1, 1])
    y1 = tf.reshape(y1, [-1, 1])
    ...
    mu1_T = tf.reshape(tf.concat([x1, y1], axis=-1), [-1, 1, 2])
    mu2_T = tf.reshape(tf.concat([x2, y2], axis=-1), [-1, 1, 2])
    ...
    item2 = -2*math.pi*tf.sqrt(det)*tf.log(1/(tf.sqrt(tf.linalg.det(2*math.pi*tmp))))
    item3 = tf.cast(tf.linalg.matmul(tf.linalg.matmul((mu1_T+mu2_T), tf.linalg.inv(tmp)), (mu1+mu2)), tf.float32)
    item3 = tf.reshape(item3, [-1, ])*(-math.pi*tf.sqrt(det))

    # item1 = tf.reshape(item1, [-1, 1])
    # item2 = tf.reshape(item2, [-1, 1])
    # item3 = tf.reshape(item3, [-1, 1])
...
def kalman_filter_(mu1, mu2, sigma1, sigma2):
    sigma1 = tf.linalg.matmul(sigma1, sigma1)
    sigma2 = tf.linalg.matmul(sigma2, sigma2)
    sigma1_inv, sigma2_inv = tf.linalg.inv(sigma1), tf.linalg.inv(sigma2)
    n1 = tf.linalg.matmul(sigma1_inv, mu1)
    n2 = tf.linalg.matmul(sigma2_inv, mu2)
    sigma_inv = tf.linalg.matmul(sigma1_inv, sigma2_inv)
    n = n1 + n2
    sigma = tf.linalg.inv(sigma_inv)
    mu = tf.linalg.matmul(sigma, n)
    c = -0.5 * (2 * tf.log(2 * math.pi) - tf.log(tf.linalg.det(sigma1_inv))
...
def kalman_filter(mu1, mu2, sigma1, sigma2):
    sigma1_square = tf.linalg.matmul(sigma1, sigma1)
    sigma2_square = tf.linalg.matmul(sigma2, sigma2)
...
def kalman_filter2(mu1, mu2, sigma1, sigma2):
    sigma1_square = tf.linalg.matmul(sigma1, sigma1)
    sigma2_square = tf.linalg.matmul(sigma2, sigma2)
...
if __name__ == '__main__':
    from alpharotate.utils.gaussian_wasserstein_distance import get_element1, get_element4
    import pandas as pd
    writer = pd.ExcelWriter('iou_.xlsx')
    n = 1000
    x1 = np.random.uniform(0, 5, n).reshape(-1, 1)
    y1 = np.random.uniform(0, 5, n).reshape(-1, 1)
    x2 = np.random.uniform(0, 5, n).reshape(-1, 1)
    y2 = np.random.uniform(0, 5, n).reshape(-1, 1)
    w1 = np.random.uniform(20, 100, n).reshape(-1, 1)
    h1 = np.random.uniform(20, 100, n).reshape(-1, 1)
    w2 = np.random.uniform(20, 100, n).reshape(-1, 1)
    h2 = np.random.uniform(20, 100, n).reshape(-1, 1)
    theta1 = np.random.uniform(-90, 0, n).reshape(-1, 1)
    theta2 = np.random.uniform(-90, 0, n).reshape(-1, 1)
    boxes1 = np.concatenate([x1, y1, w1, h1, theta1], axis=-1)
    boxes2 = np.concatenate([x2, y2, w2, h2, theta2], axis=-1)

    xy_loss = np.abs((boxes1[:, 0] - boxes2[:, 0]) / boxes1[:, 2]) + np.abs((boxes1[:, 1] - boxes2[:, 1]) / boxes1[:, 3])
    print('--'*20)
    print(xy_loss)
    # boxes1 = np.array([[20, 15, 50, 700, -90],
    #                    [50, 50, 10, 10, -90],
    #                    [50, 50, 100, 700, -70],
    #                    [50, 50, 100, 700, -70],
    #                    [50, 50, 100, 700, -30]], np.float32)
    #
    # boxes2 = np.array([[50, 50, 100, 700, -40],
    #                    [50, 50, 10, 10, -75],
    #                    [50, 50, 100, 700, -70],
    #                    [50, 50, 100, 700, -80],
    #                    [40, 30, 10, 70, -89]], np.float32)
    ...
    #     [50, 50, 40, 40, 55],
    # ], np.float32)
    iou = iou_rotate_calculate2(boxes1, boxes2).reshape(-1,)
    print('iou', iou)  # [0.9999996 0.9999998 0.9999998 1.       ]
    print(np.argsort(np.reshape(-1*iou, [-1, ])))
    ...
    sigma1_tf4, sigma2_tf4, kf_tf4 = kf(boxes1, boxes2)
    ...
    with tf.Session() as sess:
        ...
        sigma1_tf_4, sigma2_tf_4, kf_tf_4 = sess.run([sigma1_tf4, sigma2_tf4, kf_tf4])
        # print(sigma1_tf_1)
        # print(sigma2_tf_1)
        # print('**'*10)
        # print(sigma1_tf_2)
        ...
        print('gwd', np.reshape(1-1/(np.sqrt(gwd_tf_2)+1), [-1, ]))
        print(np.argsort(np.reshape(gwd_tf_2, [-1, ])))

        print('kld', np.reshape(1-1/(np.log(kl_tf_3+1)+1), [-1, ]))
        print(np.argsort(np.reshape(kl_tf_3, [-1, ])))

        print('kfiou', np.reshape(1-kf_tf_4, [-1, ]))
        print(np.argsort(np.reshape(-1*kf_tf_4, [-1, ])))
        ...
        # print(gwd_tf_2_ / (5 + gwd_tf_2_))
        data_iou = np.array(1.-iou).reshape(-1, 1)
        data_l1 = np.abs((np.log(w1/w2)) + np.abs(np.log(w2/h2)) + np.abs(theta1/180*np.pi-theta2/180*np.pi))/3 + np.abs(x1-x2)/w1 + np.abs(y1-y2)/h1
        data_gwd = np.reshape(1 - 1 / (np.sqrt(gwd_tf_2) + 1), [-1, 1])
        data_kl = np.reshape(1 - 1 / (np.log(kl_tf_3 + 1) + 1), [-1, 1])
        data_kfiou = np.reshape(1 - 3 * kf_tf_4 + xy_loss, [-1, 1])
        data = np.concatenate([data_iou, data_l1, data_gwd, data_kl, data_kfiou], axis=-1)
