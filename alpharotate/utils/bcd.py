...
def bhattacharyya_distance(mu1, mu2, mu1_T, mu2_T, sigma1, sigma2):
    sigma1_square = tf.linalg.matmul(sigma1, sigma1)
    sigma2_square = tf.linalg.matmul(sigma2, sigma2)
    ...
    item2 = 1 / 2 * tf.log(
        tf.linalg.det(sigma_square) / tf.sqrt(tf.linalg.det(tf.linalg.matmul(sigma1_square, sigma2_square))))
    item1 = tf.reshape(item1, [-1, ])
    item2 = tf.reshape(item2, [-1, ])
    return item1 + item2


def bcd(boxes1, boxes2):
    x1, y1, w1, h1, theta1 = tf.unstack(boxes1, axis=1)
    x2, y2, w2, h2, theta2 = tf.unstack(boxes2, axis=1)
    x1 = tf.reshape(x1, [-1, 1])
    y1 = tf.reshape(y1, [-1, 1])
    ...
    BCD_distance = tf.reshape(bhattacharyya_distance(mu1, mu2, mu1_T, mu2_T, sigma1, sigma2), [-1, 1])
    return sigma1, sigma2, BCD_distance
...
if __name__ == '__main__':
    ...
    boxes1 = np.array([[50, 50, 10, 150, -30],
                       [50, 50, 10, 130, -30]], np.float32)

    boxes2 = np.array([[50, 50, 10, 150, -15],
                       [50, 50, 10, 130, -15]], np.float32)
    ...
    sigma1_tf4, sigma2_tf4, bcd_tf4 = bcd(boxes1, boxes2)
    ...
    with tf.Session() as sess:
        ...
        sigma1_tf_4, sigma2_tf_4, bcd_tf_4 = sess.run([sigma1_tf4, sigma2_tf4, bcd_tf4])
        # print(sigma1_tf_1)
        # print(sigma2_tf_1)
        # print('**'*10)
        # print(sigma1_tf_2)
        ...
        print('bcd', np.reshape(bcd_tf_4, [-1, ]))
