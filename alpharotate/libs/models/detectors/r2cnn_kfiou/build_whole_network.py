...
class DetectionNetworkR2CNNKF(DetectionNetworkBase):

    def __init__(self, cfgs, is_training):
        super(DetectionNetworkR2CNNKF, self).__init__(cfgs, is_training)
        self.anchor_sampler_r2cnn = AnchorSamplerR2CNN(cfgs)
        self.proposal_sampler_r2cnn = ProposalSamplerR2CNN(cfgs)
        self.losses = LossKF(cfgs)
    ...
    def build_loss(self, rpn_box_pred, rpn_bbox_targets, rpn_cls_score, rpn_labels,
        ...
        with tf.variable_scope('build_loss'):
            ...
            with tf.variable_scope('FastRCNN_loss'):
                reg_sigma = self.losses.kalman_filter_iou_two_stage(bbox_pred=bbox_pred,
                                                                    rois=rois,
                                                                    target_gt=target_gt,
                                                                    label=labels,
                                                                    num_classes=self.cfgs.CLASS_NUM + 1)

                reg_xy = self.losses.smooth_l1_loss_rcnn_r(bbox_pred=bbox_pred,
                                                           bbox_targets=bbox_targets,
                                                           label=labels,
                                                           num_classes=self.cfgs.CLASS_NUM + 1,
