...
from alpharotate.libs.models.losses.losses_bcd import LossBCD
...
class DetectionNetworkBCD(DetectionNetworkBase):

    def __init__(self, cfgs, is_training):
        super(DetectionNetworkBCD, self).__init__(cfgs, is_training)
        self.anchor_sampler_kl = AnchorSamplerGWD(cfgs)
        self.losses = LossBCD(self.cfgs)
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_batch_h=None, gtboxes_batch_r=None, gpu_id=0):
        ...
        if self.is_training:
            with tf.variable_scope('build_loss'):
                ...
                reg_loss = self.losses.bhattacharyya_distance_loss(rpn_box_pred, anchor_states,
                                                                   target_boxes, anchors, tau=self.cfgs.BCD_TAU,
                                                                   func=self.cfgs.BCD_FUNC)
