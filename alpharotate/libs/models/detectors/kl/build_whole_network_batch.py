...
from alpharotate.libs.models.losses.losses_kl import LossKL
...
from alpharotate.libs.models.samplers.gwd.anchor_sampler_gwd_batch import AnchorSamplerGWD


class DetectionNetworkKL(DetectionNetworkBase):

    def __init__(self, cfgs, is_training):
        super(DetectionNetworkKL, self).__init__(cfgs, is_training)
        self.anchor_sampler_kl = AnchorSamplerGWD(cfgs)
        self.losses = LossKL(self.cfgs)
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_batch_h=None, gtboxes_batch_r=None, gpu_id=0):
        ...
        rpn_box_pred = tf.concat(rpn_box_pred_list, axis=1)
        rpn_cls_score = tf.concat(rpn_cls_score_list, axis=1)
        rpn_cls_prob = tf.concat(rpn_cls_prob_list, axis=1)
        ...
        if self.is_training:
            with tf.variable_scope('build_loss'):
                labels, target_delta, anchor_states, target_boxes = tf.py_func(
                    func=self.anchor_sampler_kl.anchor_target_layer,
                    inp=[gtboxes_batch_h,
                         gtboxes_batch_r, anchor_batch, gpu_id],
                ...
                if self.cfgs.REG_LOSS_MODE == 0:
                    ...
                    reg_loss = self.losses.wasserstein_distance_loss(tf.reshape(rpn_box_pred, [-1, 5]), anchor_states,
                                                                     target_boxes, anchors, tau=self.cfgs.GWD_TAU,
                                                                     func=self.cfgs.GWD_FUNC)
                elif self.cfgs.REG_LOSS_MODE == 3:
                    reg_loss = self.losses.KL_divergence_loss(tf.reshape(rpn_box_pred, [-1, 5]), anchor_states,
                                                              target_boxes, anchors, tau=self.cfgs.KL_TAU,
                                                              func=self.cfgs.KL_FUNC)
                elif self.cfgs.REG_LOSS_MODE == 4:
                    reg_loss = self.losses.JS_divergence_loss(tf.reshape(rpn_box_pred, [-1, 5]), anchor_states,
                                                              target_boxes, anchors, tau=self.cfgs.KL_TAU,
                                                              func=self.cfgs.KL_FUNC)
                elif self.cfgs.REG_LOSS_MODE == 5:
                    reg_loss = self.losses.KL_divergence_min_loss(tf.reshape(rpn_box_pred, [-1, 5]), anchor_states,
                                                                  target_boxes, anchors, tau=self.cfgs.KL_TAU,
                                                                  func=self.cfgs.KL_FUNC)
                else:
