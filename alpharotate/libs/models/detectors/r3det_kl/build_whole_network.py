...
class DetectionNetworkR3DetKL(DetectionNetworkBase):

    def __init__(self, cfgs, is_training):
        super(DetectionNetworkR3DetKL, self).__init__(cfgs, is_training)
    ...
    def refine_stage(self, input_img_batch, gtboxes_batch_r, box_pred_list, cls_prob_list, proposal_list,
        ...
        if self.is_training:
            with tf.variable_scope('build_refine_loss{}'.format(stage)):
                ...
                refine_reg_loss = self.losses.KL_divergence_loss(refine_box_pred,
                                                                 refine_box_states,
                                                                 refine_target_boxes,
