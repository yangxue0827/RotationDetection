...
class DetectionNetworkR2CNNBCD(DetectionNetworkBase):

    def __init__(self, cfgs, is_training):
        super(DetectionNetworkR2CNNBCD, self).__init__(cfgs, is_training)
        self.anchor_sampler_r2cnn = AnchorSamplerR2CNN(cfgs)
        self.proposal_sampler_r2cnn = ProposalSamplerR2CNN(cfgs)
        self.losses = LossBCD(cfgs)
    ...
    def build_loss(self, rpn_box_pred, rpn_bbox_targets, rpn_cls_score, rpn_labels,
        ...
        with tf.variable_scope('build_loss'):
            ...
            with tf.variable_scope('FastRCNN_loss'):
                reg_loss = self.losses.bhattacharyya_distance_loss_two_stage(bbox_pred=bbox_pred,
                                                                             rois=rois,
                                                                             target_gt=target_gt,
                                                                             label=labels,
