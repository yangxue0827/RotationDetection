...
class DetectionNetworkR3DetBCD(DetectionNetworkBase):

    def __init__(self, cfgs, is_training):
        super(DetectionNetworkR3DetBCD, self).__init__(cfgs, is_training)
    ...
    def refine_stage(self, input_img_batch, gtboxes_batch_r, box_pred_list, cls_prob_list, proposal_list,
        ...
        if self.is_training:
            with tf.variable_scope('build_refine_loss{}'.format(stage)):
                ...
                refine_reg_loss = self.losses.bhattacharyya_distance_loss(refine_box_pred,
                                                                          refine_box_states,
                                                                          refine_target_boxes,
                                                                          refine_boxes, is_refine=True,
                                                                          tau=self.cfgs.BCD_TAU,
