...
from alpharotate.libs.models.detectors.gwd import build_whole_network_batch
...
class TrainRetinaNetGWD(Train):
    ...
    def main(self):
        with tf.Graph().as_default() as graph, tf.device('/cpu:0'):
            ...
            gwd = build_whole_network_batch.DetectionNetworkRetinaNet(cfgs=self.cfgs,
                                                                      is_training=True)
            ...
            self.log_printer(gwd, optimizer, global_step, tower_grads, total_loss_dict, num_gpu * cfgs.BATCH_SIZE, graph)
