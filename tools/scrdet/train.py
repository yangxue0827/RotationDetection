    ...
    def main(self):
        with tf.Graph().as_default() as graph, tf.device('/cpu:0'):
            ...
            total_loss_dict = {
                ...
                'fast_cls_loss_h': tf.constant(0., tf.float32),
                'fast_reg_loss_h': tf.constant(0., tf.float32),
                'fast_cls_loss_r': tf.constant(0., tf.float32),
                'fast_reg_loss_r': tf.constant(0., tf.float32),
                'attention_loss': tf.constant(0., tf.float32),
            ...
            self.log_printer(scrdet, optimizer, global_step, tower_grads, total_loss_dict, num_gpu, graph)

if __name__ == '__main__':

    trainer = TrainSCRDet(cfgs)
    trainer.main()