...
class TrainRetinaNetBCD(Train):
    ...
    def main(self):
        with tf.Graph().as_default() as graph, tf.device('/gpu:0'):
                        ...
                        with tf.name_scope('tower_%d' % i):
                            with slim.arg_scope(
                                ...
                                with slim.arg_scope([slim.conv2d, slim.conv2d_in_plane,
                                    ...
                                    outputs = bcd.build_whole_detection_network(input_img_batch=img,
            ...
            self.log_printer(bcd, optimizer, global_step, tower_grads, total_loss_dict, num_gpu, graph)

if __name__ == '__main__':

    trainer = TrainRetinaNetBCD(cfgs)
    trainer.main()